...
in_sgdp_del=$( cat denovo/dnsv.GRCh37.DEL.bed.stix_sgdp | awk '$6>2' | cut -f 1,2,3 | sort -u | wc -l )
denovo_dup=$( cat denovo/dnsv.GRCh37.DUP.bed.stix_sgdp | cut -f 1,2,3 | sort -u | wc -l )
in_sgdp_dup=$( cat denovo/dnsv.GRCh37.DUP.bed.stix_sgdp | awk '$6>2' | cut -f 1,2,3 | sort -u | wc -l )
denovo_inv=$( cat denovo/dnsv.GRCh37.INV.bed.stix_sgdp | cut -f 1,2,3 | sort -u | wc -l )
in_sgdp_inv=$( cat denovo/dnsv.GRCh37.INV.bed.stix_sgdp | awk '$6>2' | cut -f 1,2,3 | sort -u | wc -l )
